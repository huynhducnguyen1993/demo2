...
from .forms import Nhaphangchuaduyetgaps
...
class Nhaphangs(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        pnh_active = Phieunhaphang.objects.filter(tinhtrang=True)
    ...
    def post(self, request):
        ...
        if request.method == 'POST':
            ...
            if request.POST.get('courses4'):
                ...
                tenhang_4 = None
            nhanvien: request.POST.get('nhanvien')
            noidung = {
...
class Duyetnhaphang(LoginRequiredMixin, View):
    ...
    def post(self, request, code_id):
        ...
        if user.username == 'thangnguyen':
            if request.method == 'POST':
                if request.POST.get('duyet') == 'True':
                    phieunhaphang = Phieunhaphang.objects.get(pk=code_id)
                    form = Nhaphangchuaduyetgaps(instance=phieunhaphang)

                    form.save()
                else:
                    phieunhaphang = Phieunhaphang.objects.get(pk=code_id)
                    form = Nhaphangchuaduyetgap(id=code_id, code=code_id,
                                                noidung=phieunhaphang.noidung, kho=phieunhaphang.kho,
                                                nhacungcap=phieunhaphang.nhacungcap, username=phieunhaphang.username,
                                                nhanvien=phieunhaphang.nhanvien, ghichu=phieunhaphang.ghichu,
                                                thoigiantao=phieunhaphang.thoigiantao,
                                                thoigiannhanhang=phieunhaphang.thoigiannhanhang, tuchoi=True,
                                                trinhtrang=False, phanhoi=request.POST.get('phanhoi'))
                    if form.is_valid():
                        form.save()
